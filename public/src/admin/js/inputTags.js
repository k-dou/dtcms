...
layui.define(['jquery','layer'],function(exports){
  ...
  Class.prototype.enter = function(){
    ...
    ,options = that.config;
    options.elem.focus();
    options.elem.keydown(function(){
      var tag_arr = new Array();
      var val = options.elem.val().trim();
      // if(!$val) return false;
      $.post('/admin/set_config/getTags.html',{tag_str:val},function(data){
    ...
    $('#dt-tag-pre').on('click','a',function(){
      var tag_str = '';
      var $val = $(this).text();
      if(!$val) return false;
