 ...
 * @return array
 */
function upload($file_name = 'file',$upload_path)
{
    $res = [
        'status' => 0,
        'msg' => '',
    ...
    if($info){
        $res['status'] = 1;
        $res['data']['save_name'] = $info->getSaveName();
    }else{
        $res['status'] = 0;
 ...
 * @param array $data
 */
function dtMsg($status = 0, $msg = '', $count = 0, $data = [])
{
    $obj = [
        'code' => (string)$status,
        'msg' => (string)$msg,
    ...
    die(json_encode($obj, JSON_OBJECT_AS_ARRAY));
}
